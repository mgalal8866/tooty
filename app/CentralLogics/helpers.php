...
class Helpers
{
    ...
    public static function get_login_url($type)
    {
        $data = DataSetting::whereIn('key', [
            'store_employee_login_url', 'store_login_url', 'admin_employee_login_url', 'admin_login_url'
    ...
    public static function subscription_plan_chosen($store_id, $package_id, $payment_method, $discount = 0, $pending_bill = 0, $reference = null, $type = null)
    {
        ...
        try {
            ...
            SubscriptionBillingAndRefundHistory::where([
                'store_id' => $store->id,
                'transaction_type' => 'pending_bill', 'is_success' => 0
            ])->update([
                'is_success' => 1,
    ...
    public static  function hdcharge($var = 250)
    {
        ...
        // Define the ranges and corresponding charges starting from 250
        $ranges = [];
        ...
        foreach ($ranges as $limit) {
            if ($var <= $limit) {
                return $limit;
            }
        }

        // Return the highest charge if no range is matched
        return 20000;
